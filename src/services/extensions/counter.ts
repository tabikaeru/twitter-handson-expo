...
export class Counter {
  ...
  public onSnapshot(observable: (next: CounterSnapshot) => void) {
    Object.keys(this.shards).forEach((path) => {
      this.db.doc(path).onSnapshot((snap: firebase.firestore.DocumentSnapshot) => {
  ...
  public incrementBy(val: number): Promise<void> {
    ...
    return this.doc.collection(SHARD_COLLECTION_ID).doc(this.shardId).set(update, { merge: true })
  }
...
function getShardId(cookie: string): string {
  ...
  const shardId = firebase.firestore().collection('dammy').doc().id
